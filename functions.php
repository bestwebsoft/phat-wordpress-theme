...
function phat_sidebar() {
	register_sidebar( array(
		'name'          => __( 'Main Sidebar', 'phat' ),
		'id'            => 'phat-sidebar',
	) );
}

// Enqueues scripts and styles for front-end.
function phat_scripts_styles() {
	...
	wp_enqueue_style( 'phat-open-sans', 'http://fonts.googleapis.com/css?family=Open+Sans:400,300,300italic,400italic,600&subset=cyrillic,latin' );
	wp_enqueue_style( 'phat-bevan', 'http://fonts.googleapis.com/css?family=Bevan' );
...
function phat_comment_form( $args = array(), $post_id = null ) {
	...
	if ( comments_open( $post_id ) ) :
		...
		<div id="respond" class="comment-respond">
			<h3 id="reply-title" class="comment-reply-title"><?php comment_form_title( $args['title_reply'], $args['title_reply_to'] ); ?>
			...
			<?php if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) :
				...
				<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="comment-form"<?php echo $html5 ? ' novalidate' : ''; ?>>
					<?php // Fires at the top of the comment form, inside the <form> tag.
					...
					<?php // Fires at the bottom of the comment form, inside the closing </form> tag.
...
function phat_breadcrumbs() {
	...
	if ( is_home() || is_front_page() ) {
		if ( 1 == $show_on_home ) {
			echo '<div id="crumbs"><a href="' . $home_link . '">' . sprintf( $text['home'] ) . '</a></div>';
		}
	} else {
		echo '<div id="crumbs"><a href="' . $home_link . '">' . sprintf( $text['home'] ) . '</a> ' . $delimiter . ' ';
		if ( is_category() ) {
...
// Maintains proper layout with bwsGallery
function phat_js_crunch() {
...
add_action( 'widgets_init', 'phat_sidebar' );
add_action( 'wp_enqueue_scripts', 'phat_scripts_styles' );
...
add_action( 'wp_footer', 'phat_js_crunch' );
